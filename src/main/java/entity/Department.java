 ...
 */
 ...
 public class Department   implements PrintableMenuSelection  {
    ...
    public void addDoctor(Scanner sc) throws IllegalArgumentException {
        ...
        try {
            ...
            JSONSerialization.JsonSerialization(DepartmentStorage.departments,DataType.DOCTOR);
        }
        catch(IOException e)
        {
    ...
    /**
     * Stvara novi objekt tipa RegularDepartment
     *
     * @param sc Inicijalizirani Scanner object
    ...
    public void addPatient(Scanner sc) throws IndexOutOfBoundsException, PersonnelException,IllegalArgumentException {
        ...
        try {
                JSONSerialization.UpdateLocalDataBase(this);
        }catch(IOException e)
        {
    ...
    public void addRoom() {
        ...
        try {
            JSONSerialization.JsonSerialization(this.getRooms(), DataType.ROOM);
            JSONSerialization.JsonSerialization(DepartmentStorage.departments, DataType.ROOM);
    ...
    public Map<String,List<Patient>> groupByDoctor()
    {
        Map<String, List<Patient>> map =
                patients.stream()
                .collect(Collectors.groupingBy(patient->patient.getDoctor().getName()));
    ...
    {
        Utility.printMenuSelection(patients);
        return Optional.of(patients.stream().filter(patient -> name.equalsIgnoreCase(patient.getName())).findAny().orElseThrow(NoSuchElementException::new));
